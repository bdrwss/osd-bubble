...
> **工程决策记录（2026-08-08）**：
> 1. 淡出时长由 300 ms 定为 **280 ms**：为 60fps 帧率对齐的工程优化（280ms ≈ 16.8 帧），人眼几乎不可感知，与实现保持一致。
> 2. 淡出曲线由 ease-in 改为 **easeOutCubic**（alpha 先快后慢地消散），与《动效设计规范》及实现一致。
...
**实现补充说明（2026-08-08 定案）：**
- V1.0 实现含 `quadrant` 字段，**决定保留**：编码 0–3 = 左上 / 右上 / 左下 / 右下，默认 3（右下，等效于本节的 offset + 避让逻辑）。
- 该字段已通过设置界面"气泡默认位置"象限选择器暴露为用户配置，并纳入持久化（`quadrant` 字段，防御式解析）。
...
- 不弹任何对话框（全屏时弹窗不可见且打扰）。
- **检测方案定案（2026-08-08）**：V1.0 采用**前台窗口黑名单**（`exclude_apps`，已实现：命中黑名单的应用处于前台时不显示气泡）作为主方案；DWM 自动全屏检测（Windows `DwmGetWindowAttribute` / macOS `NSWindow.styleMask.contains(.fullScreen)`）降级为 V1.1 后期可选增强，届时黑名单保留为用户手动覆盖层。
- 理由：黑名单已覆盖主要教学/游戏场景且零误报；DWM 检测需额外轮询与跨版本兼容成本，投入产出比低，待真实用户反馈漏网场景后再投入。
